...
## Current Status (snapshot for new collaborators) — 2026-04-28 (Phase 3 complete; 5.0.4 + 5.0.5 + 5.0.7 + 5.0.8 + 5.0.9 PR 1 + **5.0.10 + 5.0.10.b DONE & DEPLOYED** + **Pre-Mainnet 5.2 / Phase 4 dealer service CODE-COMPLETE pre-deploy**; reviews #14–#19 clean; **public devnet demo LIVE at https://play.pushflip.xyz/ with adjective-noun nicknames**)
...
- **Pre-Mainnet 5.2 / Phase 4 — ZK dealer service code-complete pre-deploy 2026-04-28.** The dealer that previously ran only inside `scripts/smoke-test.ts` is now a long-running Hono daemon under [`dealer/src/service.ts`](../dealer/src/service.ts) (622 LOC), wrapping the existing [`Dealer`](../dealer/src/dealer.ts) class. Five decisions locked in (full rationale + `last_compiled` 2026-04-28 in the runbook at [`docs/wiki/operations/dealer-runbook.md`](wiki/operations/dealer-runbook.md)):
  1. **Dedicated dealer keypair** (Option C): when the production game is initialized, `DEALER_PUBKEY` env var on `init-game.ts` plants a dedicated `pushflip-dealer.json` pubkey into the on-chain `dealer` field. Same blast-radius separation as the 5.0.7 faucet keypair — tucker compromise yields "can re-shuffle this game's deck", not arbitrary token movement. The `dealer` field of the live `game_id=2` GameSession is the user's CLI wallet for now (no `set_dealer` instruction exposed); production deploy creates a fresh game at a new id.
  2. **Polling auto-commit** (5s tick, single-game v1): `startAutoCommitLoop` polls `getAccountInfo` every `POLL_INTERVAL_MS=5000`, decodes the GameSession, and triggers `commitDeckForGame` when `!round_active && !deck_committed && active_player_count >= 2 && round_number !== committedRoundNumber`. The `commitInFlight` mutex is claimed BEFORE the first `await` so two `setInterval` ticks cannot race past the check while one is mid-fetch. The reset branch checks `!roundActive && !deckCommitted` to distinguish "round ended" from "committed-waiting-for-start" (both have `roundActive=false` on chain — see `start_round.rs:50,109` for the round-number bump and `end_round.rs:243-244` for the dual flag clear). No manual `POST /commit` operator-override endpoint is exposed (heavy-duty review #19 H3, see below).
  3. **`commit_deck` submission extracted** to [`dealer/src/commit-tx.ts`](../dealer/src/commit-tx.ts): single `commitDeckForGame({dealer, dealerSigner, rpc, sendAndConfirm, gameId})` helper runs shuffle → build `commit_deck` ix with `COMMIT_DECK_COMPUTE_LIMIT=400_000` CU bump → blockhash → sign → `sendAndConfirm` confirmed → return `{signature, gameSession}`. `smoke-test.ts` keeps its inline copy for now (folding it in is a separate cleanup; not load-bearing for the dealer deploy).
  4. **Frontend `useGameActions.hit()` wired** ([app/src/hooks/use-game-actions.ts](../app/src/hooks/use-game-actions.ts:461-507)): the previously-stubbed `hit` mutation now reads on-chain `gs.roundNumber + gs.drawCounter`, fetches the matching reveal from `GET /api/dealer/reveal/:gameId/:roundNumber/:leafIndex` (5s timeout, schema-validated payload, 7×64-char lowercase hex Merkle proof decoded to `Uint8Array[]`), then submits the `hit` instruction. Pre-flight chain read catches a desynced dealer before the user pays gas. `resolveDealerUrl` mirrors `resolveFaucetUrl` / `resolveNicknameBaseUrl` — `VITE_DEALER_URL` required in production, refused if cross-origin, defaults to `http://localhost:3002` in dev. `HEX_SIBLING_HASH` regex hoisted to module scope per biome's `useTopLevelRegex`.
  5. **nginx route `/api/dealer/*` → `127.0.0.1:3002`**: full diff against the live `play.pushflip.xyz.conf` inlined in the runbook's "nginx diff" section so a future deploy doesn't need to re-derive it. Separate rate-limit zone (`dealer_req`, 20r/s burst 30 — generous because `/reveal` is hit per-action during a live round). Dealer service binds explicitly to `127.0.0.1:3002` so nginx is the only ingress despite the pod's `Network=host`.

  **Heavy-duty review #19 (1 Critical / 3 High / 3 Medium, all fixed pre-commit)**:
  - **C1**: `dealer/Dockerfile`'s workspace-protocol rewrite list omitted `dealer/package.json` — `npm install` would have failed with `EUNSUPPORTEDPROTOCOL` on `workspace:*` (Lesson #55 redux). Fixed: `dealer/package.json` added to the rewrite array.
  - **H1**: auto-commit loop's reset branch fired whenever `local isRoundActive && !on-chain roundActive` — but on-chain state between `commit_deck` and `start_round` is exactly that pattern, so the very next 5s tick would have wiped the local Merkle tree right after committing it. Fixed: reset gated on `!roundActive && !deckCommitted` (round actually ended), with `deckCommitted` plumbed through `RoundSnapshot`.
  - **H2**: `commitInFlight` mutex was checked at function top BEFORE the first `await fetchGameSession`, only set ~30 lines later. Two `setInterval` ticks could pass the check while one was mid-fetch and both reach `await commitDeckForGame(...)` in parallel — `dealer.shuffle()` mutates instance state, so concurrent shuffles would corrupt each other. Fixed: mutex claimed BEFORE the first await; all early-returns + the commit path now live inside one `try { ... } finally { commitInFlight = false }`.
  - **H3**: a draft `POST /commit/:gameId` "operator override" was reachable unauthenticated through the public `/api/dealer/*` nginx prefix and lacked the player-count guard the auto-loop has (an attacker could force a 1-of-N commit, burning ~30s of dealer CPU + SOL per commit). Fixed: endpoint deleted entirely; if needed later, re-add behind a shared-secret header (`X-Dealer-Operator: ...`) plus the same player-count check.
  - **M1**: dealer bound to `0.0.0.0` by `@hono/node-server`'s default — fixed with explicit `hostname: "127.0.0.1"`.
  - **M2**: hex-validator regex was case-insensitive (`/^[0-9a-f]+$/i`) but the error message claimed "lowercase hex" — fixed by dropping the `i` flag (the dealer always emits lowercase via `Buffer.toString("hex")`).
  - **M3**: `dealer/Dockerfile` comment referenced `DEALER_ZK_ARTIFACTS_DIR` but the actual env var is `ZK_ARTIFACTS_DIR` — fixed.

  **Container infra ready, deploy gated on operator action**: `dealer/Dockerfile` mirrors `faucet/Dockerfile` after Lessons #54-#56 (`npm install` with `--ignore-scripts` + retry loop + `--mount=type=cache,target=/root/.npm` + `--network=host` on `podman build`); the runbook's quadlet template + nginx diff + deploy steps are spelled out. The next step is the operator running `solana-keygen new --outfile ~/.config/solana/pushflip-dealer.json`, transferring 0.5 SOL, initializing a fresh game with `DEALER_PUBKEY=<new-pubkey>` set, then folding the dealer build into `scripts/deploy-tucker.sh` so `bash scripts/deploy-tucker.sh` rebuilds three containers. **The current live `game_id=2` deploy at https://play.pushflip.xyz/ continues running with the old dealer field (= CLI wallet); switching it requires a new game id since `set_dealer` isn't exposed**. The 5.0.10 demo banner stays mounted until the dealer container goes live. Commit chain: `801e230 feat(dealer): productionize ZK dealer as long-running HTTP service` + `3601ee8 docs(dealer): document Pre-Mainnet 5.2 locked-in decisions + nginx diff`.

  **Known gap (acknowledged for follow-up)**: the dealer service has no automated tests of the auto-commit state machine. Three of the #19 findings (H1, H2, H3) were state-machine bugs that targeted unit tests would have caught. Tracked as a follow-up — adding regression coverage is the next thing on the list.
...
- **Pre-Mainnet 5.0.7 — DONE 2026-04-15.** Self-service test-`$FLIP` faucet shipped. New `faucet/` workspace: a Hono HTTP service that holds the mint authority and mints tokens to any recipient wallet via `POST /faucet`. The faucet pays all fees, so brand-new wallets with 0 SOL receive tokens without needing an airdrop first — this was the explicit design choice over the on-chain-faucet-program alternative (which would have required the user to have SOL to call it). Backend includes: env-driven config with friendly-error boot-time validation, per-recipient cooldown (default 24h) + in-flight guard via `pendingClaims: Set<string>` (closes the concurrent-request bypass that the 17th heavy-duty review flagged as Critical), error-message sanitization (raw RPC errors logged internally, generic message to client), boot-time balance check with `process.exit(1)` if < 500K lamports, CORS locked to `ALLOWED_ORIGINS`, own `biome.jsonc`. Frontend: new `useFaucet` hook returns a discriminated-union result (`ok`/`rate_limited`/`error`) so every terminal state is explicit; `VITE_FAUCET_URL` resolution throws in production builds when unset (closes the High silent-localhost-fallback finding); new `<NoAtaFaucetPanel>` subcomponent inside `<JoinGameDialog>` replaces the previous "message the maintainer" block with an interactive one-click flow. **Also shipped `TEST_FLIP_MINT` consolidation**: single canonical export from `@pushflip/client/constants.ts`, re-exported from `scripts/devnet-config.ts` and `app/src/lib/constants.ts` (previously triplicated with drift risk). **Live on devnet**: happy-path tx `2bHKUDJKgHkCXgPfh1sps8VhMntHHFPRpsheGZvYFY8nmpgSjF2ynogvdooftq55aws44YLSt1YbDYZFfc42VJaZ`; concurrent-race fix verified empirically (5 parallel POSTs for the same wallet → 4 × 429, 1 × 200).
- **Public devnet demo LIVE at https://play.pushflip.xyz/ — DEPLOYED 2026-04-26/27.** First public-facing deploy of pushflip. Stack: rootless podman containers via systemd quadlets on tucker (Panmoni's production VPS), behind the existing rootful nginx container (Network=host on the loopback) with Let's Encrypt + auto-renew. Frontend served by `serve -s dist` on `127.0.0.1:5175`; faucet (Hono) on `127.0.0.1:3001` proxied at `/api/faucet`. Same-origin path routing, no CORS surface. Helius free-tier devnet RPC wired into both the Vite bundle (build-time `VITE_RPC_ENDPOINT`/`VITE_RPC_WS_ENDPOINT` ARGs) and the faucet (runtime `RPC_ENDPOINT`/`WS_ENDPOINT` from `.env.production`) — public devnet RPC's 1 req/s cap would have visibly broken a single concurrent user. **Dedicated faucet keypair** generated and `$FLIP` mint authority transferred from the user's CLI wallet (`3XXMLDEf…`) to a new keypair (`5vzyxxJ1NwoN5PgX1p2zCavbxc7mugLMdF7At5syGfA6`) shipped to tucker — blast-radius separation, not a personal-wallet keypair on the VPS. Authority-transfer tx: `5GR6KHASrRtRPqbqCwgXbk3nH3vBKZaLRXpMRTegXKuF9guHmv6My5bKifqCGNvnzP7z56TcNBPRfTfkT8pHN1f1`. Faucet wallet ~9.99 SOL, cooldown=1440min, amount=1000 `$FLIP`. **One-shot redeploy via `./scripts/deploy-tucker.sh`** (~25 s end-to-end on cache-warm builds): pre-flight ssh + disk check, tag previous images as `:prev` for rollback, `git fetch + reset --hard origin/main`, `pnpm install --frozen-lockfile`, two `podman build`s (vite + faucet), `systemctl --user daemon-reload && restart pushflip-pod pushflip-vite pushflip-faucet`, then a retry-with-backoff curl smoke-check against the public URL + `/api/health`. The script prints a self-verifying rollback command on success and on failure. Companion UI shipped: a slate-palette `<DemoStageBanner>` ([app/src/components/misc/demo-stage-banner.tsx](../app/src/components/misc/demo-stage-banner.tsx)) explaining "you can connect, mint, join — full gameplay needs the not-yet-deployed dealer" (sessionStorage-dismissible), and a Panmoni footer attribution mirroring the tokenstork pattern (`A <panmoni.svg> Panmoni project`). **Container build was the long pole**: pnpm@10 + libuv worker threads in containerized Node deadlocks at ~733/734 packages installed, on both `node:20-alpine` (musl) and `node:20-slim` (glibc). Pinning `pnpm@9` in the Dockerfiles cleared the deadlock — same pathology yapbay-vite hit on this same host months ago, where they switched to `npm ci`. Three other deploy-script issues caught and fixed in flight: postinstall scripts (biome) hung the install (fixed with `--ignore-scripts` since we don't lint inside the deploy image); `systemctl restart` killed the pod via `ExecStopPost=podman pod rm` and the containers' `BindsTo=pushflip-pod.service` then failed to start (fixed by including `pushflip-pod` in the script's restart group); smoke checks raced ahead of upstream port binding by ~1–3 s (fixed with retry-with-backoff 2s/4s/6s/8s, max 5 attempts) — the same race re-bit the rollback printer until that loop also got embedded. **Lesson #54 records the multi-step fix narrative.** SSL auto-renewal verified active across all 4 certs on tucker (`certbot.timer` + `certbot renew --dry-run` clean). nginx site config lives in the separate `server-config` repo (decoupled from pushflip redeploys by design — a pushflip deploy can never break unrelated tenants on tucker via a nginx-config typo). **Outstanding (small)**: browser E2E from a fresh wallet still owed; one stray typo'd image tag (`pushflip-faucetatest:latest`) reappeared on tucker after cleanup, possibly regenerated from podman build history — cosmetic. Full deploy plan + commit-by-commit fix history at [docs/DEPLOYMENT_PLAN.md](DEPLOYMENT_PLAN.md).
- **Pre-Mainnet 5.0.5 — DONE 2026-04-15.** Fleshed out the three stub wiki pages from 100 total lines to ~650 lines of real content (commit `1b8b04b`). **Quickstart** ([docs/wiki/getting-started/quickstart.md](wiki/getting-started/quickstart.md), tutorial): zero-to-working-devnet-loop in ~20 minutes — prereqs table, clone + install, wallet + airdrop, program build, smoke test with expected CU output (commit_deck 86K/200K, hit 9.5K/12K), init-game at `game_id=1`, dev server, test $FLIP mint, local wiki preview, 8-item troubleshooting block. **Threat Model** ([docs/wiki/architecture/threat-model.md](wiki/architecture/threat-model.md), explanation): the consolidated security story for the current devnet phase — per-role trust table (Authority/Dealer/House/Treasury/Players) with blast radius, full attack-surface audit instruction-by-instruction, what the ZK proof does and does NOT prove (valid permutation vs. random shuffle; game_id-not-bound cross-game proof reuse risk tracked in 5.0.2), `vault_ready` design rationale with security properties, 7 heavy-duty review catches (review #5 C1/H1/M1/M3 + Task 3.2 double-click + Lesson #42 BigInt + Lesson #40 publicKey), mitigations-status matrix, explicit out-of-scope list. **Dealer Runbook** ([docs/wiki/operations/dealer-runbook.md](wiki/operations/dealer-runbook.md), how-to): operational guide for the first 24/7 dealer deployment — deployment topology, `.env` template, systemd unit + docker-compose block, expected baselines table (proof-gen ~18K ms, commit_deck ~86K CU, hit ~9.5K CU), 6 error-recovery scenarios each with symptom + cause + fix, observability (logs + metrics + Grafana skeleton), disaster-recovery matrix. Also added `round_active` and `STAYED/BUST` entries to the [Glossary](wiki/architecture/glossary.md) (16th review's only finding — Medium). Health check 20/20 pages pass; mkdocs `--strict` build clean for the new pages.
- **Working tree at end of session 2026-04-27:** clean, all commits pushed to `origin/main` and live at https://play.pushflip.xyz/. The 2026-04-26 → 2026-04-27 deploy session commit chain (chronological, all on `main`) is:
  - `117d50e build(deploy): scripts/deploy-tucker.sh — one-command production deploy (Pre-Mainnet 5.0.7 / Phase 4.1)` — initial deploy script + Dockerfiles + `.dockerignore` + nginx site config + quadlet template.
  - `4db68da fix(deploy): pnpm install --ignore-scripts in Dockerfiles to unblock build (Pre-Mainnet 5.0.7 / Phase 2.1 fix)` — biome postinstall hung the container install.
  - `ac55bd6 fix(deploy): switch Dockerfile build base from node:20-alpine to node:20-slim (Pre-Mainnet 5.0.7 / Phase 2.1 fix v2)` — alpine/musl theory; turned out wrong.
  - `e6ce2d4 fix(deploy): pin pnpm@9 in Dockerfiles (Pre-Mainnet 5.0.7 / Phase 2.1 fix v3)` — actual fix; pnpm@10 deadlocks libuv worker threads in containerized Node regardless of base image.
  - `e85eec3 feat(app): footer "A Panmoni project" attribution` — followed the tokenstork Footer pattern + copied `panmoni.svg` into `app/public/`.
  - `5db67ce` (rolled into 117d50e — bringing `pushflip-pod` into the SERVICES restart group so `BindsTo` resolves correctly).
  - `927df52` (rolled into 117d50e — `smoke_check()` retry with backoff 2s/4s/6s/8s, max 5 attempts).
  - `5d784a7` (rolled into 117d50e — `print_rollback_cmd()` self-verifies via the same retry loop; `CONTAINER_IMAGES` split from `SERVICES` so pods skip the `:prev` tagging step).
  - `349d682 feat(app): demo-stage banner explaining current deploy scope` — slate-palette dismissible banner, sessionStorage persistence.

The 2026-04-12 → 2026-04-15 Pre-Mainnet 5.0.9 + 5.0.4 + 5.0.8 session commit chain is:
  ...
  - `f37231e` docs(plan): record Pre-Mainnet 5.0.4 + 5.0.8 completion + 15th review + Lesson #51
  - `1b8b04b` docs(wiki): flesh out Quickstart + Threat Model + Dealer Runbook (Pre-Mainnet 5.0.5)
  - `0ef4c36` chore(claude): allowlist awk lessons-section filter
  - `7b46a64` docs(plan): record Pre-Mainnet 5.0.5 completion + 16th review + Lesson #52
  - (this commit) feat(faucet): self-service test-`$FLIP` backend + JoinGameDialog wire-up (Pre-Mainnet 5.0.7)
...
- **All workspaces green** (re-verified 2026-04-28 end of dealer-productionization session): `clients/js` **81/81 tests**, `dealer` 11/11 tests (regression-coverage gap noted under 5.2 above), `app` 5/5 tests + typecheck / biome lint (now flags the `useTopLevelRegex` violation that #19's M2 fix addressed) / build clean, `dealer` typecheck clean, `scripts` typecheck clean (now four devnet CLIs — `init-game.ts` with `DEALER_PUBKEY` env support, `mint-test-flip.ts`, `init-vault.ts`, smoke-test.ts), `cargo check --all-targets` not re-run this session (no Rust changes since 5.0.9 PR 1's deploy 2026-04-15).

### What's next

The Phase 3B frontend is **fully complete** — every sub-task from 3.1 through 3.7.4 has shipped and been empirically verified on devnet. The Pre-Mainnet 5.0.7 faucet + 5.0.10 nicknames are **live publicly** at https://play.pushflip.xyz/. **Pre-Mainnet 5.2 / Phase 4 dealer service is code-complete pre-deploy** as of 2026-04-28 (heavy-duty review #19 clean; commits `801e230`, `3601ee8`). The next meaningful milestone is the dealer deploy itself, gated on operator action: generate the dedicated `pushflip-dealer.json` keypair, init a fresh game with `DEALER_PUBKEY=<new>`, fold the dealer build into `scripts/deploy-tucker.sh`, ship.

**Immediate follow-ups from the 2026-04-28 dealer-productionization session:**
- **Add automated tests for the dealer auto-commit state machine** (next thing on the list). Three of #19's findings (H1 deck-committed reset, H2 mutex-before-await, H3 unauth /commit removal) were state-machine bugs that targeted unit tests would have caught. Mock RPC + Dealer + sendAndConfirm; assert the loop's transitions across init / committed-not-started / round-active / round-ended / various player counts; assert the mutex blocks racing ticks.
- Generate the dedicated dealer keypair, transfer SOL, init fresh game (Decision #1 trigger).
- Fold `pushflip-dealer` build + restart into `scripts/deploy-tucker.sh` (`SERVICES`, `CONTAINER_IMAGES` arrays + the third `podman build --network=host` step mirroring the faucet).
- Apply the runbook's nginx diff to `play.pushflip.xyz.conf` in the `server-config` repo + reload nginx.
- After dealer is live: delete `<DemoStageBanner>` + its mount in `app.tsx`.

**Older follow-ups (still relevant):**
- Browser end-to-end test from a fresh wallet (connect → faucet → join → hit) — was already owed for the 5.0.7 deploy; will close with the dealer deploy since the full loop becomes possible then.
- Investigate stray typo'd image tag (`pushflip-faucetatest:latest`) on tucker if it persists.
- Phase 5.5 deferred items (Helius Developer-tier upgrade trigger, Cloudflare orange-cloud re-enable, monthly podman prune cron, keypair rotation SOP, GitHub Actions CI, Prometheus monitoring) — wait for triggers; don't pre-emptively build any of these.
...
| **Heavy-duty reviews** | Run `/heavy-duty-review` after any non-trivial commit chain. Pass 2 (verification) MUST diff agent claims against actual file contents AND session state — Pass 1 agents lie confidently. | See Lessons #20, #29, and #50. **Seventeen heavy-duty reviews so far**; every one has caught at least one structural bug the unit tests missed. The 17th review caught a genuine Critical (rate-limit race in the faucet) that Pass 2 verified empirically on devnet — NOT a "plausible ghost." Also caught a High (prod localhost fallback) + 4 Mediums, all fixed in-session. |
| **Pre-mainnet items** | Nine items deferred from Phase 3 to Phase 5. Status as of 2026-04-15: Task 5.0.1 (oversized program data slot) open; Task 5.0.2 (threshold randomness) open; Task 5.0.3 (final full-scope review) open; Task 5.0.4 (`parseU64` + script-helpers extraction) **DONE 2026-04-15** — `parseU64` half landed 2026-04-11, scripts-extraction half landed today with -712 lines net; Task 5.0.5 (three stubbed wiki pages) **DONE 2026-04-15** — Quickstart + Threat Model + Dealer Runbook all fleshed out, 100 → 650 lines; Task 5.0.6 (promote `$FLIP` with metadata + multisig) open; Task 5.0.7 (self-service test-FLIP faucet) **DONE 2026-04-15** — Hono backend at `faucet/` + in-app `<NoAtaFaucetPanel>`; Task 5.0.8 (Biome lint rule for `useWallet().publicKey` in hook deps) **DONE 2026-04-15** — GritQL plugin at `app/biome-plugins/no-publickey-in-hook-deps.grit`; Task 5.0.9 (replace state-diff event feed with on-chain `pinocchio_log` events) **PR 1 DONE 2026-04-15**, PR 2 frontend rewrite pending 24h bake. **5 of 9 items shipped this session**; 4 remain. Listed at [Pre-Mainnet Checklist](#pre-mainnet-checklist-deferred-items-not-blocking-devnet). | None block devnet work. |
...
52. **The 16th heavy-duty review (2026-04-15) cleared the Pre-Mainnet 5.0.5 wiki diff — and produced the 6th "plausible ghost" in a month.** Reviewed the three new wiki pages (Quickstart + Threat Model + Dealer Runbook, ~650 lines of new content) with a docs-tuned 2-pass methodology: Pass 1A verified every specific factual claim against the actual codebase state (toolchain versions, `pnpm` script names, PDAs, the mint address, CU budget constants, Merkle depth, VK fingerprint, review-#5 fix letter designations); Pass 1B verified link resolution + frontmatter compliance + terminology consistency + contradiction-between-pages checks + whether speculative/future content was clearly framed. Result: 0C / 0H / 1M / 0L. The single Medium was two terms used in Threat Model (`round_active`, `STAYED/BUST`) missing from the Glossary — fixed in-session by adding both entries. **Pass 1A Agent A confidently flagged "CRITICAL: Threat Model claims '51 lessons as of 2026-04-15' but only ~11 exist"** — dismissed in Pass 2 after a 5-second `awk '/### Lessons Learned/,/^### /' | grep -c '^[0-9]+\.'` that returned 51. The agent had grepped for `"Lesson #N"` *cross-references* (a small subset, used to point back at named lessons from other sections) instead of counting the numbered entries in the Lessons Learned section itself. This is **the 6th "plausible ghost"** (Lesson #50) — reviews #9, #10, #13, #14, #15, #16 — and the second in a row against *my own work* (the 14th flagged `hit.rs`, the 16th flagged this plan doc). The Pass 2 discipline: don't trust a single grep from an agent against a claim you made deliberately; reach for the mechanical counter (awk-over-section, tsc --noEmit, biome check, etc.) before accepting the refutation. Also notable: Pass 1B's meta-check on speculative content in the Dealer Runbook — "A reader landing on this page today should not think `systemctl start pushflip-dealer` works." The review confirmed every `/health` endpoint, systemd unit, and production-deploy mention was correctly framed as aspirational, not as current behavior.

53. **The 17th heavy-duty review (2026-04-15) caught the first non-"plausible ghost" Critical in weeks — a real concurrent-request bypass in the faucet rate limiter.** Reviewed the Pre-Mainnet 5.0.7 faucet diff (9 new files + 4 modified). Result: 1C / 1H / 4M / 2L, all fixed in-session. The Critical — **rate-limit race condition** — was flagged independently by two Pass 1 agents (security + correctness): the `checkRateLimit → mintToRecipient (1-2s await) → recordClaim` sequence is non-atomic, so N parallel POSTs for the same wallet all see `allowed: true` before any of them record a claim. Attack: `Promise.all([...Array(10)].map(() => fetch(...)))` gets N× the faucet amount. Fix: introduce `pendingClaims: Set<string>` + `tryAcquireClaim` / `releaseClaim` API that check-and-add atomically (safe under Node's single-threaded event loop — `has` / `add` on a Set are synchronous so no two handlers race). **Verified empirically on devnet**: 5 parallel POSTs for the same wallet produced 4 × 429 `"Another request for this wallet is already in flight"` + 1 × 200 with a single mint tx. A pre-fix run (accidentally performed against a stale process) had produced 5 responses with only 2 distinct signatures — empirical confirmation that the pre-fix version was exploitable. The High — **production localhost fallback** — was `VITE_FAUCET_URL ?? "http://localhost:3001/faucet"`; a production build with the env var unset would send every user's browser to their own machine. Fixed by making the fallback DEV-only and throwing at module load in production. The 4 Mediums: `TEST_FLIP_MINT` triplicated across 3 hardcoded locations (consolidated into `@pushflip/client`), faucet outside Biome scope (added `faucet/biome.jsonc`), error messages leaking RPC/authority details (sanitized to generic client-facing message), warn-only boot balance check (changed to `process.exit(1)` if < 500K lamports). **What distinguishes this review from the previous five**: Pass 1 agents caught a *real* exploitable bug, not a plausible ghost. The "two independent agents flag the same issue" signal is worth attending to — a single agent can be wrong with confidence, but two agents from different scans converging on the same specific line + attack vector is high-signal. Pass 2's verification strategy here was also different: instead of re-reading the code, **run the exploit**. 5 parallel POSTs demonstrated the race empirically before the fix and demonstrated the fix's correctness after. The "run the exploit, don't just reason about it" discipline complements the CU regression guard pattern from Pre-Mainnet 5.0.9 PR 1 step 4 — empirical verification beats code review when the consequence is financial.
...
55. **[SUPERSEDED by Lesson #56 — the entire pnpm→npm migration was a workaround for a netavark networking bug, not for any real pnpm/npm difference.] Pre-Mainnet 5.0.10 redeploy (2026-04-27) — the `pnpm@9 + node:20-slim + --ignore-scripts` workaround stack from Lesson #54 collapsed the moment the lockfile changed. Switching to `npm install` (the actual yapbay escape hatch) was a 4-step migration, not a 1-line fix.** Adding `better-sqlite3@^11.7.0` + transitive deps (`bindings`, `node-addon-api`, `prebuild-install`, …) to the faucet workspace was the trigger; the same 7.8% CPU + zero forward progress + all-threads-`S(sleeping)` signature returned, this time hanging at "added 734 of 735" in the *vite* image build (which doesn't even depend on better-sqlite3 — pnpm's filtered install still synchronizes the workspace virtual store, which materializes the new lockfile entries even with `--filter @pushflip/app --filter @pushflip/client`). Three workarounds had stacked over Lesson #54 (pnpm@9, node:20-slim, --ignore-scripts) and the bug returned anyway, which is the strongest possible evidence that the right fix is *replacing* the pnpm-in-container path entirely, not patching it further. **The npm migration in detail (4 steps, all required):** (a) **Add a `workspaces` field to root `package.json`.** pnpm prefers `pnpm-workspace.yaml` when both exist (verified — pnpm-lock.yaml didn't churn after the addition), so local pnpm dev is unaffected; npm reads the field exclusively. Without this, npm doesn't discover the workspace structure and sibling deps fail to resolve. (b) **Don't switch source files from `workspace:*` to `*`.** Tried this first; pnpm interprets `*` as "fetch from npm registry" and the local install died with `ERR_PNPM_FETCH_404 GET https://registry.npmjs.org/@pushflip%2Fclient`. pnpm requires the explicit `workspace:` marker to prefer the local workspace over the registry. So `workspace:*` MUST stay in source for pnpm dev. (c) **Rewrite `workspace:*` → `file:../<path>` at Dockerfile build time, not in source.** A Node one-liner in the Dockerfile reads each workspace's package.json after copy, swaps `workspace:*` for npm-readable `file:` paths in dependencies + devDependencies, and writes back. npm understands `file:`-as-symlink for sibling workspace folders. Source repo stays pnpm-native; the per-Dockerfile rewrite is hermetic to the container build. (d) **Update build/CMD invocations.** `pnpm --filter @pushflip/app build` → `npm run build --workspace=@pushflip/app`; `CMD ["pnpm", "start"]` → `CMD ["npm", "start"]`. Both packages exposed `start`/`build` scripts that don't depend on the package manager binary directly, so this was mechanical. **First failed deploy attempt (98f8733) confirmed step (c) is non-negotiable**: `npm install` failed loud with `EUNSUPPORTEDPROTOCOL: Unsupported URL Type "workspace:": workspace:*` — npm 10.8.2 and 11.13.0 both reject the protocol, even when the workspace itself is present in the `workspaces` array. The `workspace:` protocol is pnpm-specific. The user's "update npm to 11 on tucker" reflex didn't help — and wouldn't have, because the npm executing the install runs *inside* the `node:20-slim` container, not on the host. **Meta-lesson on workaround-stacking**: each Lesson #54 patch (pnpm@9 pin, slim base, --ignore-scripts) addressed a *symptom* of the same underlying pnpm/libuv bug. Stacking three symptom-fixes feels like progress but creates a brittle stack — a small lockfile change collapses all three at once. The yapbay escape hatch ("just use npm") was always available; we held off because it required ~80 lines of Dockerfile change + a workspace-protocol rewrite. That cost was higher than each individual workaround, but lower than the cumulative cost we paid stacking workarounds. **Heuristic going forward**: if a workaround has been stacked twice already and the bug recurs, switch tools rather than adding a fourth patch.

56. **5.0.10 deploy saga, finale (2026-04-27/28) — eight stacked workarounds were all band-aids on a netavark networking bug; `--network=host` on `podman build` was the one-flag actual fix.** After Lesson #55's npm migration cleared the `EUNSUPPORTEDPROTOCOL` error, every deploy attempt hit `npm error code EIDLETIMEOUT — Idle timeout reached for host registry.npmjs.org:443`. Five additional workaround layers stacked over the pnpm→npm switch over the next ~5 hours of deploy attempts: (a) `--fetch-retries=5 --fetch-timeout=600000` on `npm install` (no effect — those flags configure individual fetches, not the underlying agent socket); (b) `--mount=type=cache,target=/root/.npm` (no effect on cold-cache first run, the moment that mattered); (c) drop `--omit=optional` (necessary fix for an unrelated rolldown native-binding error, but didn't address the timeouts); (d) shell retry loop with `--prefer-offline` so each retry would resume from the cache (no effect — every attempt of every retry hit EIDLETIMEOUT at roughly the same point); (e) `bash -c` escaping correctness in the retry loop. Each fix was patient, well-reasoned, and covered the wrong layer. **The diagnostic move that cracked it was a 30-second test outside the build**: `ssh tucker 'curl https://registry.npmjs.org/react'` returned 200 in **0.37s, 6.6 MB**. Ten parallel manifest fetches: all 200 OK. Tucker's network was perfect; the failures were entirely inside the build container's network namespace. The actual bug: podman's rootless **netavark** networking stack mangles long-lived TCP keep-alives in a way that breaks npm's HTTP-agent connection pool — same registry, same host, same packages, but routed through the rootless container's NAT, sockets idle out at ~60s. Fix: `podman build --network=host` runs the build container directly on the host's network stack (same as how the runtime `pushflip` pod already runs `Network=host` per its quadlet, which is why the runtime services have never had this problem — only the *build* containers did). One flag. Install completed in **43s on the first attempt of the retry loop**, never needed retry #2. Total deploy time after the fix: 2m 51s. **What this also explains in retrospect**: Lesson #54's `pnpm@9` fix worked at the time not because pnpm@9 was correct and pnpm@10 was buggy, but because the smaller package set in 5.0.7 was less likely to trigger socket idle timeouts during the install window. The 5.0.10 redeploy added enough new packages (better-sqlite3 + ~5 transitive deps) that the install runtime crossed the netavark idle threshold and the bug returned. Lesson #54's `pnpm@9` pin and Lesson #55's entire pnpm→npm migration were both workarounds for symptoms of *this same* underlying bug. Both of those lessons should be read with that in mind — Lesson #54 is now flagged as superseded, and the npm migration (#55) is a "kept because it's already shipped, but not load-bearing" change. **Meta-meta-lesson**: when a class of failure (deploy-time install hangs, with one symptom in pnpm and another in npm) keeps recurring across tool switches, the bug is below the tool layer. Drop down a layer (network, syscall, kernel) and instrument from there. Heuristic: **3+ stacked workarounds across 2+ tool changes ⇒ stop fixing the tool; the bug is in the layer below.** Same shape as #55's "switch tools rather than adding a fourth patch" but applied to the system layer, not the tool itself. **Operationally**: ~7 hours of stacking band-aids before stepping outside the container. The diagnostic that should have come first (test connectivity from a layer below) cost 30 seconds. Cost-of-band-aid math: each individual workaround feels cheaper than "really debug this" but the cumulative cost is dramatically higher. Force the from-below diagnostic earlier next time. **The list of layers we kept, sorted by what they actually fix**: `--network=host` is the only correctness-critical one; `--ignore-scripts` is required because biome postinstalls remain disk-heavy; `workspace:*` → `file:` Dockerfile rewrite is required because we now use npm; `--mount=type=cache,target=/root/.npm` is a perf optimization (warm rebuilds drop ~40s → ~5s on the install layer); the rest (`--fetch-retries`, `--fetch-timeout`, shell retry loop, `--prefer-offline`) are belt-and-suspenders that cost nothing on success and provide resilience against future flakes. Full commit-by-commit walkthrough in [docs/DEPLOYMENT_PLAN.md](DEPLOYMENT_PLAN.md).
...
##### 5.0.5: Documentation debt — flesh out 3 wiki stubs — ✅ DONE 2026-04-15

**Status:** Shipped as commit `1b8b04b`. All three stub pages fleshed out (100 lines → ~650 lines). Health check 20/20 pages pass; mkdocs `--strict` build clean for the new pages. Glossary updated with `round_active` + `STAYED/BUST` entries (16th heavy-duty review's only finding — Medium, fixed in-session).

**What shipped:**

1. **[docs/wiki/getting-started/quickstart.md](wiki/getting-started/quickstart.md)** (tutorial) — zero-to-working-devnet-loop in ~20 minutes. Prereqs table, clone + install, devnet wallet setup, program build, smoke test with expected CU output (commit_deck 86K/200K, hit 9.5K/12K), persistent game-init at `game_id=1`, dev server, test $FLIP mint, local wiki preview, 8-item troubleshooting block.

2. **[docs/wiki/architecture/threat-model.md](wiki/architecture/threat-model.md)** (explanation) — the consolidated security story for the current devnet phase. At-a-glance summary, per-role trust table (Authority/Dealer/House/Treasury/Players) with blast radius, full attack-surface audit instruction-by-instruction, what the ZK proof does and does NOT prove (valid permutation vs. random shuffle; game_id-not-bound cross-game proof reuse risk tracked in 5.0.2), `vault_ready` design rationale with security properties, 7 heavy-duty review catches (review #5 C1/H1/M1/M3 + Task 3.2 double-click + Lesson #42 BigInt + Lesson #40 publicKey), mitigations-status matrix, explicit out-of-scope list.

3. **[docs/wiki/operations/dealer-runbook.md](wiki/operations/dealer-runbook.md)** (how-to) — operational guide for the first 24/7 dealer deployment. Deployment topology, environment setup with `.env` template, systemd unit + docker-compose block, expected baselines table (proof-gen ~18K ms, commit_deck ~86K CU, hit ~9.5K CU), 6 error-recovery scenarios each with symptom + cause + fix (OOM, RPC disconnect, blockhash expiry, backlog, keypair compromise, proof-state-lost), observability (logs + metrics + Grafana skeleton), disaster-recovery matrix. Clearly framed as "the runbook for the first 24/7 deployment" — today the dealer only runs via smoke-test.

**Original context (for historical reference):** The MkDocs Material wiki shipped at `docs/wiki/` on 2026-04-11. Three pages were intentionally stubbed so the nav was structurally complete without inflating the migration scope. The stubs pointed at where the real content temporarily lived (`CONTRIBUTING.md`, `README`, FAQ). Filling them in was deferred until each topic had enough distilled knowledge to justify its own page — the Threat Model in particular waited until the 14-review backlog of catches gave it a real narrative spine instead of a thin placeholder.
...
##### 5.0.7: Self-service test-FLIP faucet — ✅ DONE 2026-04-15

**Status:** Shipped. New `faucet/` workspace is a Hono HTTP service; new `useFaucet` hook + `<NoAtaFaucetPanel>` wire it into the JoinGameDialog. Option 1 (backend service) was chosen over option 2 (on-chain permissionless mint program) specifically because the backend pays all tx fees — a brand-new 0-SOL wallet can click "Get test $FLIP" and receive tokens without needing to `solana airdrop` first. That smoothest-possible-first-user flow was the explicit user-stated priority.

**What shipped:**

- **Hono HTTP service** at [`faucet/src/server.ts`](../faucet/src/server.ts): `POST /faucet` (mint) + `GET /health` (liveness + authority balance). CORS locked to `ALLOWED_ORIGINS` allowlist. Error responses are generic ("Mint failed. Please retry in a moment."); raw RPC errors are logged internally only.
- **Atomic rate limiter** at [`faucet/src/rate-limit.ts`](../faucet/src/rate-limit.ts): per-recipient cooldown (default 24h) PLUS a `pendingClaims: Set<string>` in-flight guard. The in-flight guard was the 17th heavy-duty review's Critical fix — without it, N parallel POSTs for the same wallet all clear the cooldown check before any of them record a claim, draining the faucet by Nx. **Verified empirically on devnet**: 5 parallel requests → 4 × 429, 1 × 200 with tx `2bHKUDJKgHkCXgPfh1sps8VhMntHHFPRpsheGZvYFY8nmpgSjF2ynogvdooftq55aws44YLSt1YbDYZFfc42VJaZ`.
- **Env-driven config** at [`faucet/src/config.ts`](../faucet/src/config.ts) with friendly-error boot-time validation (ENOENT/EACCES/malformed-JSON on the keypair file). Boot-time balance check: `process.exit(1)` if authority has < 500K lamports (~100 mints worth).
- **`useFaucet` hook** at [`app/src/hooks/use-faucet.ts`](../app/src/hooks/use-faucet.ts) returns a discriminated-union result (`ok`/`rate_limited`/`error`) so every terminal state is explicit. `VITE_FAUCET_URL` resolution throws at module load in production builds when unset (closes the 17th review's High silent-localhost-fallback finding); localhost default is dev-only.
- **`<NoAtaFaucetPanel>` subcomponent** in JoinGameDialog replaces the previous "message George with your address" copy with a single interactive "Get test $FLIP" button + pending/success/rate-limited/error states. Extracted to keep JoinGameDialog under the cognitive-complexity lint threshold.
- **`TEST_FLIP_MINT` consolidation**: single canonical export from [`clients/js/src/constants.ts`](../clients/js/src/constants.ts); `scripts/devnet-config.ts` and `app/src/lib/constants.ts` re-export from there. Prevents the 3-way drift that the 17th review flagged.
- **Own `biome.jsonc`** extending `ultracite/biome/core` (+ `lint` / `lint:fix` scripts) so the faucet is under the same lint discipline as the app.

**Out of scope / deferred (documented in [`faucet/README.md`](../faucet/README.md) security section):**
- Global rate limit (per-wallet only; sybil resistance is not a goal for devnet test tokens).
- Durable rate-limit store (in-memory Map; process restart wipes cooldowns).
- Metrics / Prometheus endpoints (add alongside the dealer runbook deploy in Phase 5).
- Mainnet-ready keypair rotation procedure.

**Original plan context (for historical reference):**

**Original context:** Today, getting test $FLIP into a wallet requires the test mint authority (George's local CLI keypair) to manually run [`scripts/mint-test-flip.ts`](../scripts/mint-test-flip.ts) (added 2026-04-11) against the recipient's address. That works for a single tester but doesn't scale to "anyone who lands on devnet wants to try the game" — every new user is a manual touchpoint for George.
...
5. **Token-account-redirect sweep audit (GitHub [PR #2](https://github.com/Panmoni/pushflip/pull/2) follow-up) — DONE 2026-04-25.** PR #2 fixed a Critical token-account-redirect vulnerability in `end_round` (caller-supplied destination + no owner check → any player in `turn_order` could pass their own ATA on the same mint as `winner_token_account` and drain the pot). Reviewed every SPL Token `Transfer` CPI in [program/src/instructions/](../program/src/instructions/) for the same vulnerability class. **Four Transfer sites total, all SAFE:** three in [end_round.rs](../program/src/instructions/end_round.rs) (lines 188 / 201 / 224) now guarded by `verify_token_account` after PR #2; one in [join_round.rs](../program/src/instructions/join_round.rs) at line 150 transfers from a player-supplied ATA to a deterministic vault PDA derived from `game_session` — destination is not caller-controlled, so the redirect class doesn't apply. Bounty instructions (`add_bounty`, `claim_bounty`, `close_bounty_board`), `leave_game`, and the burn instructions (`burn_second_chance`, `burn_scry`) do no Transfer CPIs at all (burns use `Burn`, account closes use rent-refund lamport transfers, claim_bounty records on-chain only). Two independent rejection tests landed in [tests/src/phase2.rs](../tests/src/phase2.rs): `test_end_round_rejects_wrong_owner_winner_ata` (locks down the winner-payout wire at `end_round.rs:200`) and `test_end_round_rejects_wrong_owner_treasury_ata` (locks down the rake wire at `end_round.rs:187`). Both assert `Custom(33)` (`InvalidTokenAccount`) via a centralized `INVALID_TOKEN_ACCOUNT_CODE` constant + verify post-rejection balances on vault, attacker ATA, and treasury (the rake CPI fires before the winner check rejects, so atomic rollback of treasury is the strongest invariant locked down). Setup deduplicated into `setup_finished_round` helper. Phase2 suite now 13/13.

##### 5.0.10: Display names — deterministic nicknames + optional SNS — Phases 1-3 CODE-COMPLETE 2026-04-27, deploy pending

**Status (2026-04-27):** All three frontend-facing phases shipped at the code level. Awaiting tucker redeploy to land in production. Phase 4 (SNS overlay) remains deferred until a mainnet build exists. **18th heavy-duty review clean** (0 Critical / 0 High / 2 Medium / 9 Low — all 11 fixed in-session before this writeup).

**What shipped (Phases 1–3):**

- **Phase 1 — Faucet-side registry.** New `faucet/src/nicknames/` module: `words.ts` (256 adjectives × 256 nouns, both length-power-of-2 with module-load assertions), `db.ts` (better-sqlite3 with WAL + `synchronous=FULL`, `assertSchemaInvariants` boot check, `NICKNAME_ALGORITHM_VERSION=1`), `assign.ts` (deterministic SHA-256 probe sequence + `INSERT OR FAIL` retry on the UNIQUE constraint, idempotency fast path, `ProbeLimits` injectable for tests). New `GET /nickname/:address` endpoint in `server.ts` (idempotent registration); `POST /faucet` registers the recipient as a side effect. New deps: `better-sqlite3@^11.7.0` + `@types/better-sqlite3`. **17 node:test cases** cover determinism, idempotency, forced collision, 200-pubkey global-uniqueness sweep, schema-version invariants, restart persistence, full Cartesian profanity sweep, Phase B suffix fallback (with shrunk `ProbeLimits`), and exhaustion-throw behavior.

- **Phase 2 — Frontend hook + component.** New `app/src/hooks/use-display-name.ts` (React Query with `staleTime: Infinity` + `gcTime: Infinity`, 3 s `AbortSignal.timeout`, no-retry-on-error policy) and `app/src/components/misc/display-name.tsx` (single-purpose `<DisplayName>` rendering registry name → SNS → truncated `4…4` fallback). Production build refuses non-relative `VITE_NICKNAME_URL` to close the cross-origin exfiltration class (same defense added to `VITE_FAUCET_URL` in passing).

- **Phase 3 — UI sweep.** Replaced inlined `shortAddress` copies in `wallet-button.tsx`, `game-board.tsx`, `turn-indicator.tsx` with `<DisplayName>`. Refactored `event-render.ts`'s `renderEventMessage(event): string` → `renderEventTokens(event): EventToken[]` so `event-feed.tsx` can render `<DisplayName>` JSX inline. Added `renderEventMessageText` companion for sonner toasts (which fire outside the React tree). New `app/src/lib/address-format.ts` consolidates the canonical `truncateAddress` (folds in the 5.0.4 deferred follow-up #4). Added `app/src/lib/event-render.test.ts` with the load-bearing "≤ 1 address per kind" invariant test that walks all 16 event kinds.

- **Container + deploy plumbing.** `faucet/Dockerfile` runs `pnpm rebuild better-sqlite3` after the `--ignore-scripts` install, with a `node -e "require('better-sqlite3')"` smoke check that fails loudly with an actionable error if upstream ever drops the prebuild for our (node-20, linux-x64, glibc) target. Quadlet on tucker bind-mounts `/home/george9874/repos/pushflip/faucet/data` rw so `nicknames.db` survives container restarts; `Environment=NICKNAME_DB_PATH=…` set on the quadlet (avoids touching `.env.production`). `scripts/deploy-tucker.sh` adds `--build-arg VITE_NICKNAME_URL=/api/nickname` next to the existing `VITE_FAUCET_URL` arg.

**Live state** (after deploy lands): faucet boots with `[faucet] nickname registry: N row(s) at /…/nicknames.db algorithm=v1` banner; `/health` reports `nickname_count: N`; first user to hit `GET /nickname/:address` for an unregistered wallet gets `assigned: true`, all subsequent calls return `assigned: false` with the same nickname.

**Pre-deploy E2E (local):** real-wallet smoke produced `bubbly-crystal` for `AczL…MDjH` and `topaz-seagull` for `CAgD…XDxb` (the two wallets visible in the user's pre-PR screenshot). `/health` reported `nickname_count: 2` after the two-call sweep.

**Outstanding:** the `<DisplayName>` first-paint fetches against the registry add a ~30–80 ms round-trip per distinct address. React Query dedupes concurrent calls and caches `Infinity`, so the cost is paid once per address per session; with localStorage persistence (already declared but not yet wired to a `@tanstack/query-sync-storage-persister`), the cost would amortize to zero on reload. Wiring the persister is a small follow-up — not blocking the deploy. Tracked separately as 5.0.10.b.



**Supersedes 5.0.4 deferred follow-up #4** ("Consolidate the four `shortAddress` copies") — that consolidation is folded into Phase 3 below.

**Context.** The public devnet demo (live since Pre-Mainnet 5.0.7) surfaces wallet addresses in five places: the connected-wallet pill in the header, every player row on the game board, the active-player badge in the turn indicator, the join/hit/stay/burn lines in the event feed, and the in-flight claimer in the JoinGameDialog no-ATA flow. Each reads the same `slice(0,4) + '…' + slice(-4)` rule. The result is unreadable — `UoZh…naxa joined`, `CAgD…XDxb hit — BUST` — and indistinguishable across opponents in a 4-seat round. We want recognisable identities so the event feed feels like watching named players, not hex.

The user's framing: "stored in the db but not editable… if the user wants to override it they get an SNS name." Plus an explicit requirement added during planning: **nicknames must be globally unique** — guaranteed, not probabilistic. That rules out "pure client-side hash → adjective+noun" because two distinct pubkeys can hash to the same `(adjective_index, noun_index)` pair. Strict global uniqueness requires a server-side registry that owns the assignment. The faucet service is the natural home for it (it already authenticates the address, runs on tucker, and is the only other long-lived component besides the chain). It gains a small SQLite store; nicknames are first-come-first-served; once assigned, an `address → nickname` row is permanent and can never be changed by the user. The only way to display something other than the auto-assigned nickname is to register an SNS domain that resolves back to the same wallet.

There is **no existing SNS or display-name infrastructure** in the codebase. Truncation lives in five inlined `shortAddress` copies (`game-board.tsx`, `turn-indicator.tsx`, `wallet-button.tsx`, plus the canonical one in [`app/src/lib/event-render.ts`](../app/src/lib/event-render.ts) whose docstring already calls out the drift hazard).

**Decisions.**

- **D1 — SNS lookup target cluster: defer SNS to a Phase 4 follow-up; do nicknames-only for the devnet demo.** SNS records live on **mainnet-beta**; the frontend's RPC client (`app/src/lib/program.ts`) talks to devnet only. Adding a second mainnet RPC just for vanity display names introduces a second rate-limited public endpoint, a CORS-prone third-party fetch in the critical path, and a Bonfida program-owner dependency. The deterministic nickname covers 100% of users out-of-the-box; SNS coverage is small (single-digit-percent of devnet wallets) and adds latency + flicker. Re-evaluate when there's a mainnet build to attach it to. Phase 4 below leaves the door open behind a feature flag.

- **D2 — Assignment algorithm: hash-seeded probing on a server-owned registry.** Word lists vendored as small inline TypeScript arrays at `faucet/src/nicknames/words.ts` (~256 adjectives × ~256 nouns ≈ 65K bare combinations). Pure client-side hash → `adjective-noun` would give a ~1.8% birthday collision rate at 50 users, which violates the global-uniqueness requirement. Algorithm: the faucet service computes `seed = SHA-256(pubkey-bytes)`. The candidate sequence is `cand_i = (adjectives[h(seed, i) % 256], nouns[h(seed, i+1) % 256])` for `i = 0, 1, 2, …`. The first candidate not present in the `nicknames` table wins; the row is committed atomically. If all `(adj, noun)` pairs ever collide (impossible until > 65K users), append a numeric suffix `-2`, `-3`, … by extending the probe with a counter. **Why a deterministic seed even though the registry is authoritative**: it makes the assignment auditable and reproducible — for any pubkey we can replay the probe sequence offline and verify the assigned nickname is the first-free candidate at the moment of registration. Adjective+noun was chosen over the user's "Mikey + Baby" suggestion because (i) `brave-otter` reads as a handle, not as a real person impersonated; (ii) curated adjective/noun lists are easier to filter for profanity; (iii) handle-style names match the playful demo tone. Lists derived from public-domain word lists (Docker `moby-friendly-name` / Heroku haikunator / GitHub repo-name suggester all use this exact shape) — vendor a derived list rather than depend on `unique-names-generator` to keep the bundle tight. **Profanity filter:** banlist enforced at module-load in dev; bigger risk is *combinations* like `craven-loser` — both individually OK, but the pair stings. Mitigation: noun list deliberately skips negative-trait nouns (`loser`, `failure`, `idiot` are out); adjective list skips clearly-pejorative adjectives (`craven`, `pathetic`, `worthless` are out). The output should never insult the holder.

- **D3 — Registry storage: SQLite file inside the faucet service.** New file `faucet/data/nicknames.db` with a single table `nicknames(address TEXT PRIMARY KEY, nickname TEXT UNIQUE NOT NULL, assigned_at INTEGER NOT NULL)`. Use `bun:sqlite` (Bun is the faucet's runtime; native SQLite, zero extra dep) or `better-sqlite3` if running under Node. Concurrency: the existing `pendingClaims: Set<string>` pattern (5.0.7's faucet rate-limit fix) already prevents two concurrent registrations for the *same* address; concurrent registrations of *different* addresses both targeting the same first-choice nickname are resolved by SQLite's `UNIQUE` constraint on `nickname` — the loser catches the constraint violation and retries with the next probe candidate. Storage footprint: ~80 bytes per row; 1M users ≈ 80 MB, fits comfortably on tucker. Quadlet bind-mount at `/home/george9874/repos/pushflip/faucet/data/` so the file survives container restarts. **Backups**: weekly `cp` to a sibling directory is enough for the devnet demo (rebuilding the table from on-chain history isn't possible — assignments are first-come-first-served and not deterministic across re-runs once the table is lost). For mainnet, daily backup + write-ahead-log mode.

- **D4 — Faucet API surface: one new endpoint, idempotent.** `GET /api/nickname/:address` returns `{ nickname: string, source: 'registered' }`. If the address is not yet in the table, the endpoint registers it inline (using the probing algorithm) and returns the freshly-assigned nickname. Idempotent — calling twice returns the same answer. The same registration path also fires inside `POST /faucet` as a side-effect, so any wallet that ever claimed test `$FLIP` is registered. Rate-limited (separate from the per-recipient cooldown — registration has no economic cost; cap at ~10 RPS per IP via `limit_req` in the existing nginx site config). CORS already locked to `https://play.pushflip.xyz` via the same `ALLOWED_ORIGINS` allowlist the faucet uses.

- **D5 — Frontend caching: React Query + localStorage persistence.** Pattern follows [`app/src/hooks/use-token-balance.ts`](../app/src/hooks/use-token-balance.ts) — `queryKey: ['displayName', addressBase58]`, `staleTime: Infinity` (assigned nicknames are permanent), `gcTime: Infinity`. Persist the cache to localStorage via `@tanstack/query-sync-storage-persister` so a hard-refresh doesn't re-fetch the same addresses we already saw — keeps the cold-paint clean. The fetch is a single `GET /api/nickname/:address`; React Query dedupes concurrent requests for the same key automatically. **Loading-state UX**: the first time an address appears in the UI, the hook returns `isLoading: true` for the round-trip (~30–80 ms typical against same-origin nginx). The component renders a width-stable skeleton during that window. Subsequent renders for the same address are synchronous from cache. **Offline / faucet-down fallback**: if the API is unreachable for > 3 s, fall back to the truncated `slice(0,4)…slice(-4)` form for the duration of that session, with a warning logged to console. The fallback is **not** persisted to the cache — next reload retries the registry. Permanent fallback to truncated would silently violate the uniqueness invariant.

- **D6 — SNS overlay (deferred — Phase 4): no SNS dep needed for Phases 1–3.** When/if Phase 4 happens, use `@bonfida/spl-name-service` (the canonical SNS resolver from the Bonfida team that runs the program). Bundle-size cost ~40 KB gzipped (acceptable, but argues for lazy-loading inside the SNS hook so the cost is paid only once a mainnet RPC is wired). Verify against Context7 docs at the time of Phase 4 implementation.

- **D7 — Component shape: `<DisplayName address={pubkey} />` + `useDisplayName(address)` hook.** Hook returns `{ name: string; isLoading: boolean; source: 'nickname' | 'sns' | 'fallback' }`. Phase 1–3 returns either `source: 'nickname'` (registry hit) or `source: 'fallback'` (registry unreachable, truncated address rendered). Phase 4 layers SNS on top: `source: 'sns'` if the user has a registered .sol that reverse-resolves to this address. Component is a thin wrapper that renders a width-stable skeleton during loading. Caller-side concerns stay caller-side: the `(you)` suffix on `game-board.tsx` is composed by the caller (`<DisplayName address={addr} />{isMe ? ' (you)' : ''}`).

**Phased plan.**

- **Phase 1 — Faucet-side registry: word lists + assignment algorithm + endpoint + tests (~90 min).** New files in the faucet package: `faucet/src/nicknames/words.ts` (length-power-of-2 arrays, 256 each — clean bit-mask, no modulo bias), `faucet/src/nicknames/assign.ts` exporting `assignNickname(address: string, db: Database): Promise<string>` (computes the deterministic probe sequence from `SHA-256(pubkey-bytes)` using Bun's `crypto`; tries each candidate against the SQLite `UNIQUE` constraint with `INSERT OR FAIL`; on conflict, advances the probe; numeric suffix `-2`/`-3`/… only after exhausting all 65K bare candidates), `faucet/src/nicknames/db.ts` (sqlite open + table-creation idempotency), `faucet/src/server.ts` adds `GET /api/nickname/:address` (rate-limited via existing nginx zone). Faucet tests: determinism of the probe sequence given a fixed seed, idempotency (same address → same nickname across calls), uniqueness invariant (1000 random addresses → 1000 distinct nicknames; force a collision by seeding two addresses to the same first-choice nickname and assert the loser gets the second-choice), banned-word filter. Quadlet update: bind-mount `/home/george9874/repos/pushflip/faucet/data/` so `nicknames.db` survives container restarts. No frontend changes. Mergeable on its own — the endpoint is silent until Phase 2 wires it up.

- **Phase 2 — `useDisplayName` hook + `<DisplayName>` component (~60 min).** New `app/src/hooks/use-display-name.ts` modelled on `use-token-balance.ts`: React Query with `queryKey: ['displayName', addressBase58]`, `staleTime: Infinity`, `gcTime: Infinity`, `enabled: address !== null`; queryFn calls `GET /api/nickname/:address` (3 s timeout via AbortController; on timeout/error returns `{ name: truncate(address), source: 'fallback' }` and does NOT cache the fallback — `throwOnError: false` + `select` post-processing). Persistence: register `@tanstack/query-sync-storage-persister` against localStorage with a `pushflip:display-name-cache:v1` key; only persist queries whose `source === 'nickname'` (or `'sns'` post-Phase-4). New `app/src/components/misc/display-name.tsx` (~25 lines, plain `<span>` with width-stable skeleton during the round-trip; the skeleton width estimates ~12ch which is the typical adjective+noun length). Test asserts the hook returns the registered nickname, returns the truncated fallback on faucet-down, and doesn't persist the fallback. No callers updated yet. Mergeable on its own.

- **Phase 3 — Replace truncated-address renders across the app (~75 min).**
  - [`app/src/components/wallet/wallet-button.tsx`](../app/src/components/wallet/wallet-button.tsx) — delete local `shortAddress`, render `<DisplayName>` inside the connected pill.
  - [`app/src/components/game/game-board.tsx`](../app/src/components/game/game-board.tsx) — delete local `shortAddress`. The conditional `${shortAddress(addr)}${isMe ? ' (you)' : ''}` becomes `<><DisplayName address={addr} />{isMe ? ' (you)' : ''}</>`.
  - [`app/src/components/game/turn-indicator.tsx`](../app/src/components/game/turn-indicator.tsx) — delete local `shortAddress`, render `<DisplayName>`.
  - [`app/src/lib/event-render.ts`](../app/src/lib/event-render.ts) — refactor `renderEventMessage(event): string` → `renderEventTokens(event): EventToken[]` where `type EventToken = string | { kind: 'address'; address: Address }`. `event-feed.tsx` interleaves `<DisplayName>` JSX accordingly. Bumps Phase 3 by ~30 min but eliminates implicit React-Query-cache coupling (the alternative — `queryClient.getQueryData(['displayName', addr])` lookups inside a `string`-returning renderer — has a cache-miss footgun). The exhaustive `never` check in the existing event-kind switch catches missed kinds at compile-time.
  - [`app/src/components/game/player-hand.tsx`](../app/src/components/game/player-hand.tsx) — currently takes a pre-formatted `label: string` from its caller. Update prop type to `label: ReactNode`; caller (game-board.tsx) feeds `<DisplayName />` JSX.
  - JoinGameDialog + `<NoAtaFaucetPanel>` — verify in implementation; add `<DisplayName>` if any address renders are found.
  - After this phase, `shortAddress` in `event-render.ts` has only the **fallback** caller (the hook's offline path). Keep it but rename to `truncateAddress` and re-export from `app/src/lib/`. `hexPubkeyToAddress` stays unchanged.

- **Phase 4 (deferred — gated on mainnet milestone) — SNS overlay.** Add `VITE_SNS_RPC_URL` env var (mainnet-beta endpoint, separate from devnet `VITE_RPC_URL`). Lazy-import `@bonfida/spl-name-service` inside the queryFn so Phase 1–3 builds don't pull it. Behind feature flag `VITE_ENABLE_SNS=1` so we can ship dark and flip on after a bake. `useDisplayName` becomes: try SNS reverse-lookup against mainnet → if hit, return SNS name (`source: 'sns'`); if miss/error/timeout (3 s), fall back to the registry nickname. 24h `staleTime` for SNS specifically. Add a hover-tooltip showing the full base58 address so users can copy/paste — worth doing regardless.

**Critical files.**
- `faucet/src/nicknames/words.ts`, `faucet/src/nicknames/assign.ts`, `faucet/src/nicknames/db.ts` (new — Phase 1)
- `faucet/src/server.ts` (add `GET /api/nickname/:address` route — Phase 1)
- `faucet/data/.gitkeep` + `~/.config/containers/systemd/pushflip-faucet.container` quadlet update (bind-mount the data dir — Phase 1)
- `app/src/hooks/use-display-name.ts` (new — Phase 2)
- `app/src/components/misc/display-name.tsx` (new — Phase 2)
- `app/src/lib/event-render.ts` (refactor — Phase 3, tokenize the message)
- `app/src/components/game/event-feed.tsx` (caller change — Phase 3)
- `app/src/components/game/game-board.tsx`, `app/src/components/wallet/wallet-button.tsx`, `app/src/components/game/turn-indicator.tsx`, `app/src/components/game/player-hand.tsx` (delete inlined `shortAddress` / take `ReactNode` label — Phase 3)

**Existing patterns to reuse.** Faucet's existing `pendingClaims: Set<string>` atomic-acquire pattern (5.0.7) for the per-address registration mutex. Faucet's existing env-validated `config.ts` shape — add `NICKNAME_DB_PATH` with a sane default of `./data/nicknames.db`. React Query hook shape from [`use-token-balance.ts`](../app/src/hooks/use-token-balance.ts) (queryKey factory exported for invalidation, base58-string-as-key per Lesson #40 — the 5.0.8 lint plugin catches backsliding). Vitest harness already configured at both the `app/` and `faucet/` scope; no new tooling.

**Verification.** Visual: header pill, player rows, turn indicator, event feed all show distinct memorable nicknames per seat. **Global-uniqueness invariant**: a startup test inside the faucet runs `SELECT COUNT(*) - COUNT(DISTINCT nickname) FROM nicknames` on every boot and `process.exit(1)` if the result is non-zero — turns the SQLite `UNIQUE` constraint into a load-bearing runtime check, not just a schema annotation. Reload-stability: connect a wallet, note nickname, hard-refresh + reconnect → identical nickname (registry hit + localStorage persistence keep the wire trip out of the cold-paint path). Cross-browser stability: open the same wallet in a second browser → identical nickname (both clients hit the same registry row). Concurrent-registration test: spawn 5 parallel `GET /api/nickname/:address` for two distinct addresses that happen to share a first-choice nickname; assert exactly one address gets the bare name and the other gets the next probe candidate, with no duplicate-row behaviour. Profanity: snapshot review of the full Cartesian product `ADJECTIVES × NOUNS` once at PR time, then frozen. Layout-stability: DevTools "Highlight layout shifts" while connecting/disconnecting — pill width changes once when nickname resolves (one-time shift, not flicker). Faucet-down fallback: temporarily block `/api/nickname/*` in DevTools network throttling, reload — UI shows truncated `UoZh…naxa` form, console warns once, no localStorage write of the fallback. No event-feed regressions: smoke-test via a full round, confirm all 17 event kinds still render correctly after the tokenization refactor.

**Risks.**
- **Faucet becomes stateful** — currently the only persistent state is the keypair file (mounted read-only). Adding `nicknames.db` means the faucet has data that's costly to lose (re-running `INSERT OR FAIL` against an empty DB silently re-assigns nicknames, breaking the cross-session stability invariant for every existing user). Mitigation: bind-mount the `data/` dir from the host so it survives container restarts; document weekly `cp` backup; add a startup log line listing the row count so a "wait, why is it 0?" surprise is visible immediately.
- **Single point of failure for display names** — if the faucet is down, every wallet renders as the truncated fallback for the duration of the outage. Acceptable for the demo; for mainnet, consider replicating the registry to read replicas or moving to a CDN-cacheable static endpoint.
- **Probe-sequence determinism is load-bearing** — if the algorithm changes between deploys, two callers computing "what should this address's nickname be?" can disagree (the registry is the source of truth, but auditing tools that re-derive offline would diverge). Mitigation: version the algorithm in a constant `NICKNAME_ALGORITHM_VERSION = 1`; store on each row; refuse to mix versions in the same DB.
- **Word-list licensing**: derived public-domain lists are fine; document the source in `words.ts` header.
- **Combinatorial profanity**: noun + adjective curation handles it; snapshot review is the backstop.
- **Identity-theft / impersonation**: nicknames are scoped to pushflip's UI only; the actual identity is the pubkey, always. The faucet's `UNIQUE` constraint guarantees no two pubkeys ever share a nickname, so a malicious party can't "claim" an impersonating nickname for a different wallet.
- **DB-store choice (`bun:sqlite` vs `better-sqlite3`)** — current faucet runs under Bun (per `faucet/package.json`); `bun:sqlite` is the natural pick. Verify at implementation time the production runtime is still Bun — if it's been switched to Node, swap to `better-sqlite3`. Both have the same SQL surface, the swap is a 5-line change.
- **SNS lookup latency (Phase 4)**: 200–800 ms typical against a public mainnet RPC; mitigated by 3 s timeout + registry-nickname fallback + 24h `staleTime`.
- **Mainnet RPC rate limits (Phase 4)**: a busy room with 8 players = 8 RPCs on first paint. React Query dedupes identical keys. If we proceed to Phase 4, batch via Bonfida's bulk-resolver or self-host the lookup in the faucet service.
- **Tokenization refactor blast radius**: Phase 3's `renderEventMessage` → `renderEventTokens` touches all 17 event kinds. The exhaustive `never` check + `tsc --noEmit` is the safety net. Run `pnpm --filter @pushflip/app test` + a manual smoke round before merging.

**Plan readiness.** Estimated wall-clock for Phases 1–3 (the user-facing scope): **~3.75 hours**, splittable as `P1 (90m) → P2 (60m) → P3 (75m)`. Phase 1 is the largest because it's the new faucet-side functionality (DB + endpoint + concurrency-correct algorithm + tests). Each phase is independently mergeable. Phase 4 (SNS) is **deferred** — no estimate until a mainnet build exists for it to attach to.
