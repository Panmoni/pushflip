...
- **Phase 3B (frontend) Tasks 3.1 → 3.7.1: COMPLETE.** Vite 8 + React 19 + TypeScript 6 + Tailwind v4 + Biome/Ultracite scaffold, shadcn primitives, `WalletProvider` + `QueryProvider` + Kit RPC clients, the workspace-wide `@solana/kit` 2.x → 6.x migration, **Task 3.2 program integration hooks** (`useGameSession`, `usePlayerState`, `useGameActions` + the wallet adapter ↔ Kit signing bridge in [src/lib/wallet-bridge.ts](../app/src/lib/wallet-bridge.ts)), and the **full Phase 3.3 → 3.7.1 frontend** all shipped 2026-04-10/11. The frontend now contains:
  ...
  - **Theme** (Task 3.7.1): `.dark` palette retuned to the brand "degen" aesthetic (deep purple primary `#9945ff`, Solana green accent `#14f195`, near-black background `#0a0a0f`). The matching `:root` light palette retuned to the same brand spine (deep purple primary, Solana green accent, near-white background) so the user can switch between them. Light/dark theme toggle in the header via `useTheme` hook + `<ThemeToggle>` component (cycles light → dark → system, persists to localStorage as `pushflip:theme`, FOUC-prevented via an inline script in `index.html` that mirrors the resolution rules before React mounts).
...
1. **`scripts/lib/script-helpers.ts` extraction** — `init-game.ts`, `mint-test-flip.ts`, and `smoke-test.ts` still duplicate `loadCliKeypair`, `printRpcError`, `sendTx`, and the ANSI `c` color table. Defer until a session that also touches `smoke-test.ts` (the Poseidon syscall regression guard) so the devnet re-verification happens in one pass.
...
| **Heavy-duty reviews** | Run `/heavy-duty-review` after any non-trivial commit chain. Pass 2 (verification) MUST diff agent claims against actual file contents AND session state — Pass 1 agents lie confidently. | See Lessons #20 and #29. **Eleven heavy-duty reviews so far**; every one has caught at least one structural bug the unit tests missed (including a Critical cross-game claim exploit, a near-miss authority gating gap, the Task 3.2 double-click double-spend pattern, and the BigInt-u64 silent-wrap footgun which has now been caught FOUR separate times — in `joinRound` stake validation, in `init-game.ts`'s GAME_ID parser, in `JoinGameDialog`'s stake parser, and in `mint-test-flip.ts`'s `--amount` parser — see Lesson #42. After the 4th occurrence, `parseU64` was consolidated into `@pushflip/client` and all four call sites migrated in one pass; see Pre-Mainnet 5.0.4). |
| **Pre-mainnet items** | Seven items deferred from Phase 3 to Phase 5: reclaim the oversized program data slot (Task 5.0.1), threshold randomness / decentralized dealer (Task 5.0.2), final full-scope review (Task 5.0.3), the shared `parseU64` helper + `scripts/lib/script-helpers.ts` extraction (Task 5.0.4 — added 2026-04-11 after the BigInt-u64 footgun re-occurred), the three stubbed wiki pages — Quickstart, Threat Model, Dealer Runbook (Task 5.0.5 — added 2026-04-11 with the wiki migration), promote `$FLIP` from a manual test mint to a permanent token with metadata + multisig authority (Task 5.0.6 — added 2026-04-11), and a self-service test-FLIP faucet (Task 5.0.7 — added 2026-04-11). Listed at [Pre-Mainnet Checklist](#pre-mainnet-checklist-deferred-items-not-blocking-devnet). | None block devnet work. |
...
**Status:** `parseU64` half ✅ **DONE 2026-04-11** (11th heavy-duty review fix pass). `script-helpers.ts` extraction + Biome lint rule still deferred.
