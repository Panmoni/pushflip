...
## Current Status (snapshot for new collaborators) — 2026-04-11 (post Phase 3.3→3.7.1 + 10th review)
...
- **Phase 3B (frontend) Tasks 3.1 → 3.7.1: COMPLETE.** Vite 8 + React 19 + TypeScript 6 + Tailwind v4 + Biome/Ultracite scaffold, shadcn primitives, `WalletProvider` + `QueryProvider` + Kit RPC clients, the workspace-wide `@solana/kit` 2.x → 6.x migration, **Task 3.2 program integration hooks** (`useGameSession`, `usePlayerState`, `useGameActions` + the wallet adapter ↔ Kit signing bridge in [src/lib/wallet-bridge.ts](../app/src/lib/wallet-bridge.ts)), and the **full Phase 3.3 → 3.7.1 frontend** all shipped 2026-04-10/11. The frontend now contains:
  ...
  - **Flip Advisor** (Task 3.5): `calculateBustProbability` + `getRecommendation` pure math in [app/src/lib/advisor.ts](../app/src/lib/advisor.ts) + `<FlipAdvisor>` component with Degen Mode toggle.
  ...
  - **Theme** (Task 3.7.1): `.dark` palette retuned to the brand "degen" aesthetic (deep purple primary `#9945ff`, Solana green accent `#14f195`, near-black background `#0a0a0f`). Dark mode activated by default via `class="dark"` on `<html>`.
  - **Shared $FLIP helpers** ([app/src/lib/flip-format.ts](../app/src/lib/flip-format.ts)): `FLIP_SCALE` + `formatFlip(amount, opts)` (full + compact modes) + `parseU64` (the canonical fix for the recurring BigInt-u64 silent-wrap footgun, mirroring the same regex-then-bounds-check pattern from `init-game.ts`). Added during the 10th heavy-duty review fix pass to consolidate three duplicate `FLIP_SCALE` constants and four nearly-identical formatter functions.
- **Five heavy-duty reviews against the frontend so far** (#6 + #7 + #8 + #9 + #10):
  - #6/#7/#8 fixed 33 findings against the Phase 3.2 hook surface
  - #9 reviewed the init-game.ts commit chain on 2026-04-11 and fixed 3 Mediums + 1 Low in the same session (BigInt-u64 silent-wrap re-occurrence in the GAME_ID env parser, missing error context extraction, missing `process.exit(0)` for the WSS handle, logging style drift)
  - **#10 reviewed the entire Phase 3.3 → 3.7.1 commit chain (19 files, ~2,100 LOC) on 2026-04-11**: 0 Critical / 0 High / 6 Medium / 6 Low. All 12 findings fixed in the same session — including the **third occurrence** of the BigInt-u64 silent-wrap footgun (in `JoinGameDialog`'s stake parser, with no upper bound on `whole * FLIP_SCALE`), a `submit-when-no-ATA` UX gap, a duplicate-toast regression in the dialog, dead semantic color tokens left over from the theme retune, and a `useRef` migration on the connection-status counter so it survives future deps-array changes. See Lessons Learned #42 for the full BigInt-u64 footgun lineage.
- **First end-to-end on-chain → frontend round-trip is live.** Read paths verified empirically against `game_id=1`. Write paths (transaction signing through the wallet bridge) remain **unverified end-to-end** — `useGameActions.joinRound` is wired into `<JoinGameDialog>`, but the first real user click against devnet still hasn't happened. That's the Phase 3.B integration test that closes Phase 3B.
- **Working tree at end of session 2026-04-11:** uncommitted changes per the user's "no commits unless asked" workflow. Modified: `app/index.html` (`class="dark"`), `app/src/app.tsx` (rewired around `<GameBoard>`), `app/src/components/game/card.tsx` (Task 3.3.1 polish), `app/src/styles/globals.css` (3.7.1 theme), plus `notes.md` (off-limits) and `.claude/settings.json` (auto-added bash allowlist). New files: 16 in `app/src/{components,hooks,lib}/`. Phase 3.3 → 3.7.1 commit chain has NOT been committed yet — pending George's manual commit process.
- **All workspaces green** (re-verified 2026-04-11 end of session): `clients/js` 26/26 tests, `dealer` 11/11 tests, `app` typecheck/lint (38 files) / build (~668ms) clean, `scripts` typecheck clean, `cargo check --all-targets` clean (same 14 + 8 + 4 = 26 deferred dead-code warnings tracked under Task 3.B.End — no new warnings from this session).

### What's next

The Phase 3B frontend is **structurally complete** through Task 3.7.1. The remaining work splits into three tracks:

1. **Phase 3B end-to-end verification** (the most important — closes Phase 3B):
   - Click `<ActionButtons>` → "Join game" → confirm in Phantom/Solflare → verify the `joinRound` transaction lands on devnet against `game_id=1` (`Hk6RLHBZ8oppV4KtQFFRsHC21z9tCL5HYz3cLELEA64A`).
   - This is the **first ever real signed transaction** through the wallet adapter ↔ Kit bridge. Until this happens, the entire Phase 3.2 + 3.3 + 3.4 surface is theoretical — every line of code is type-correct, lint-clean, and builds, but hasn't been exercised against a real wallet for a real on-chain effect.
   - Cannot proceed without test $FLIP in the user's wallet — the `JoinGameDialog`'s no-ATA branch will block submit. Need to mint test FLIP to George's devnet keypair via the test mint authority.

2. **Polish tasks 3.7.2 → 3.7.4** (deferred from this session by user instruction):
   - **3.7.2 — Animations.** Framer Motion integration: card flip on draw, shake on bust, confetti on win, pulse on "your turn". The `<GameCard>` component already accepts an `animate` prop as a placeholder.
   - **3.7.3 — Responsive layout.** Mobile / phone-viewport (375px) testing and fixes. The current layout is desktop-first.
   - **3.7.4 — Loading + error states.** Skeleton loaders, retry buttons, friendly error messages mapping program error codes to text.

3. **Phase 4 — House AI agent** (~3 days estimated). Autonomous opponent that plays against humans, integrated with the ZK dealer. See [Phase 4: The House AI Agent](#phase-4-the-house-ai-agent-days-23-25).

**Recommended next move**: ship the Phase 3.3 → 3.7.1 commit chain (currently uncommitted), then execute Track 1 (mint test FLIP + click Join in the browser). That closes Phase 3B with empirical proof of the entire stack and unblocks Phase 4.

### Pre-Mainnet items — Task 5.0.4 update

The new [`app/src/lib/flip-format.ts`](../app/src/lib/flip-format.ts) is a **stepping stone** toward Pre-Mainnet 5.0.4 (`parseU64` extraction to `@pushflip/client`). The frontend now has its own canonical `parseU64` that mirrors `scripts/init-game.ts::parseU64` exactly. The remaining work is to lift the helper into `@pushflip/client` so the dealer + scripts + frontend share one source — but the immediate problem (three duplicated copies of `FLIP_SCALE` and the BigInt-u64 footgun re-occurring in `JoinGameDialog`) is now fixed.
...
| **Heavy-duty reviews** | Run `/heavy-duty-review` after any non-trivial commit chain. Pass 2 (verification) MUST diff agent claims against actual file contents AND session state — Pass 1 agents lie confidently. | See Lessons #20 and #29. **Ten heavy-duty reviews so far**; every one has caught at least one structural bug the unit tests missed (including a Critical cross-game claim exploit, a near-miss authority gating gap, the Task 3.2 double-click double-spend pattern, and the BigInt-u64 silent-wrap footgun which has now been caught THREE separate times — in `joinRound` stake validation, in `init-game.ts`'s GAME_ID parser, and in `JoinGameDialog`'s stake parser — see Lesson #42 + Pre-Mainnet 5.0.4). |
...
42. **`new DataView(buf.buffer).setBigUint64(value, ...)` silently wraps negative bigints to huge u64s.** Verified empirically: `setBigUint64(0, -100n, true)` produces `[156, 255, 255, 255, 255, 255, 255, 255]` (= u64::MAX − 99) instead of throwing a `RangeError`. This bit Task 3.2 because `joinRound(stakeAmount: bigint)` accepts arbitrary bigints from caller code and the underlying `u64Le()` helper in `@pushflip/client` uses `setBigUint64` directly. The on-chain program would reject the wrapped value (because it exceeds the user's balance), but only after burning the user's gas. **Fix:** validate at the hook boundary BEFORE calling the instruction builder: `if (stakeAmount < MIN_STAKE) throw new Error(...);`. Also use `MIN_STAKE` imported from `@pushflip/client` so there's one source of truth across the workspace. **The general rule:** any function that takes a bigint and forwards it to a u64 encoder must validate the range explicitly. Don't trust JavaScript to throw.
   ...
   **Lineage of this footgun (now THREE separate occurrences):**
   ...
   **Three occurrences in two weeks means the shared `parseU64` helper is no longer optional** — tracked as Pre-Mainnet Checklist item [5.0.4](#504-extract-shared-parseu64-helper--scripts-lib-script-helpersts). The 10th review's fix pass extracted `parseU64` to `app/src/lib/flip-format.ts` as a stepping stone; the remaining work in 5.0.4 is to lift it into `@pushflip/client` so the dealer + scripts + frontend all share one source. Until then, the footgun WILL re-occur — only the lint rule that 5.0.4 calls out can prevent occurrence #4.
...
#### Task 3.2: Program Integration Layer — COMPLETED 2026-04-10
...
**Context:** Two distinct DRY violations surfaced together in the **ninth heavy-duty review** (2026-04-11) after [scripts/init-game.ts](../scripts/init-game.ts) shipped, and the `parseU64` half got worse during the **tenth review** when the same footgun re-appeared a third time:

1. **The `BigInt(userInput)` → `setBigUint64` silent-wrap footgun has now appeared THREE times.** First in `useGameActions.joinRound`'s stakeAmount (caught in 8th review, fix in [use-game-actions.ts](../app/src/hooks/use-game-actions.ts)). Then in `init-game.ts`'s `GAME_ID` env parser (caught in 9th review, fix in [init-game.ts](../scripts/init-game.ts) with a local `parseU64` helper). Then **a third time** in `JoinGameDialog.parseStakeInput` (caught in 10th review 2026-04-11; the dialog had the `≥ MIN_STAKE` floor but no `≤ U64_MAX` ceiling and would silently overflow on a paste of `99999999999999999999999999`). Each fix re-implements the same `/^\d+$/` + max-value validation pattern. **Stepping stone**: the 10th review's fix pass extracted a canonical `parseU64` to [app/src/lib/flip-format.ts](../app/src/lib/flip-format.ts) so the frontend has one source of truth. The remaining 5.0.4 work is to lift it into `@pushflip/client` so the dealer + scripts + frontend all share the same helper. See Lessons Learned #42 for the full lineage.
