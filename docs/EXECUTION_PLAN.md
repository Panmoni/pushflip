...
## Current Status (snapshot for new collaborators) — 2026-04-11 (Phase 3B CLOSED — first on-chain wallet-signed joinRound live; 12th heavy-duty review clean)
...
- **Phase 3B (frontend) Tasks 3.1 → 3.7.1: COMPLETE AND END-TO-END VERIFIED.** Vite 8 + React 19 + TypeScript 6 + Tailwind v4 + Biome/Ultracite scaffold, shadcn primitives, `WalletProvider` + `QueryProvider` + Kit RPC clients, the workspace-wide `@solana/kit` 2.x → 6.x migration, **Task 3.2 program integration hooks** (`useGameSession`, `usePlayerState`, `useGameActions` + the wallet adapter ↔ Kit signing bridge in [src/lib/wallet-bridge.ts](../app/src/lib/wallet-bridge.ts)), and the **full Phase 3.3 → 3.7.1 frontend** shipped 2026-04-10/11. The frontend now contains:
...
- **Seven heavy-duty reviews against the frontend so far** (#6 + #7 + #8 + #9 + #10 + #11 + #12):
  ...
  - See Lessons Learned #42 for the BigInt-u64 footgun lineage (four occurrences, now consolidated in `@pushflip/client::parseU64`).
- **Working tree at end of session 2026-04-11:** clean. All six session commit chains pushed to `main`: parseU64 consolidation + JoinGameDialog fixes (`93259f3`), mint-test-flip faucet (`ee5ff1f`), theme switcher (`c595a18`), EXECUTION_PLAN update for 11th review (`b5f8caf`), bash allowlist tooling (`a9ed311`), JoinGameDialog amber contrast (`a875b75`), wallet-bridge hardening (`3337a89`), FlipAdvisor contrast (`cf56176`), EXECUTION_PLAN update for 12th review (`f36d991`), bash allowlist for fix pass (`a40278d`), and this snapshot update.
- **All workspaces green** (re-verified 2026-04-11 end of session): `clients/js` **35/35 tests** (up from 26 after parseU64 extraction), `dealer` 11/11 tests, `app` typecheck / lint (43 files) / build (~680ms) clean, `scripts` typecheck clean, `cargo check --all-targets` clean (same 26 deferred dead-code warnings tracked under Task 3.B.End — no new warnings from this session).

### What's next

The Phase 3B frontend is **structurally complete AND empirically verified** through Task 3.7.1. The remaining work splits into two tracks:

1. **Polish tasks 3.7.2 → 3.7.4** (deferred by user instruction — can ship incrementally):
   ...
   - **3.7.3 — Responsive layout.** Mobile / phone-viewport (375px) testing and fixes. The current layout is desktop-first. Header already uses `flex-wrap` for narrow screens (wallet-bridge hardening session).
   - **3.7.4 — Loading + error states.** Skeleton loaders, retry buttons, friendly error messages mapping program error codes to text. Partial progress: `TransactionSimulationError` + `formatTxErrorDescription` already render actionable toast copy with filtered program logs, so the *error* side of 3.7.4 is substantially done — the remaining work is visual: loading skeletons + retry affordances.

2. **Phase 4 — House AI agent** (~3 days estimated). Autonomous opponent that plays against humans, integrated with the ZK dealer. See [Phase 4: The House AI Agent](#phase-4-the-house-ai-agent-days-23-25).

**Recommended next move**: Phase 3B is done; Phase 4 is the next meaningful milestone. Polish tasks 3.7.2-3.7.4 can ship as standalone PRs at any time without blocking Phase 4.
