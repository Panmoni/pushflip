...
- **Task 3.3.3 — `<ActionButtons>`** — Hit / Stay / Second Chance / Scry buttons wired to `useGameActions`. Each button must respect `isPending` for disable state (the hook's intrinsic re-entry guard catches double-clicks regardless, but the disable is the UX layer). (~20 min)
- **Task 3.3.4 — `<GameBoard>`** — container that wires `useGameSession` + `usePlayerState` + `useGameActions` together and renders `<PlayerHand>` × N + `<ActionButtons>`. **Replaces the current `<GameStatusPanel>` verification surface in [app/src/app.tsx](../app/src/app.tsx).** (~25 min)
- **Task 3.3.5 — `<PotDisplay>` + `<TurnIndicator>`** — small atomic components, animate on state change. (~15 min)

Full task spec at [Task 3.3: Game Board Components](#task-33-game-board-components). Tasks 3.3.1 and 3.3.5 are **fully independent** (pure presentational, no hooks). 3.3.2 depends on 3.3.1. 3.3.3 depends on the existing `useGameActions`. 3.3.4 depends on all of the above. **First user click on `<ActionButtons>` will be the first end-to-end test of the wallet adapter ↔ Kit signing bridge** — see [Task 3.2 result block](#task-32-program-integration-layer--completed-2026-04-10) for context.
